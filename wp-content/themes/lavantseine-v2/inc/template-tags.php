...
if ( ! function_exists( 'get_event_dates' ) ) :
	function get_event_dates($event_first_date, $event_last_date, $event_other_dates = array(), $exhibition = false) {
		...
		if( $exhibition ) {
				$event_dates .= 'Du ';
				$event_dates .= strftime('%d %B %G', $event_first_date );
				$event_dates .= ' au ';
				$event_dates .= strftime('%d %B %G', $event_last_date );
		}
		else {
		...
		if( $event_other_dates[0] != '' &&  $event_first_date != $event_last_date ) {
				...
				if( !strcmp( strftime('%A %d %b %G', $event_first_date ), strftime('%A %d %b %G', $event_last_date ) ) ) {
					...
					$event_dates .= 'Le ';
					$event_dates .= strftime('%A %d %B %G', $event_first_date );
			...
			elseif( $event_first_date != $event_last_date ) {
				...
				$event_dates .= strftime('%A %d %B %G - %kh%M', $event_first_date );
			}
		}
...
function lavantseine_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> articles plus anciens', 'lavantseine' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'articles plus récents <span class="meta-nav">&rarr;</span>', 'lavantseine' ) ); ?></div>
			<?php endif; ?>
